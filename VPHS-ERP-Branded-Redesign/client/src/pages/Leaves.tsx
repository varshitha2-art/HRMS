...
export const Leaves: React.FC = () => {
  ...
  const onLeaveTodayCount = requests.filter(r => {
    ...
    return r.startDate.slice(0, 10) <= today && r.endDate.slice(0, 10) >= today;
  }).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-600 mt-1">
            {isAdminView ? 'Review employee leave requests, approval queues and attendance sync.' : 'Apply leave, track quota balances and monitor your approved leave.'}
          </p>
        </div>
      ...
      {/* Role-aware leave overview */}
      {isAdminView ? (
        <div>
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700 mb-3">Leave Operations — 2026</h2>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ['Pending Approval', pendingCount, 'bg-amber-50 border-amber-200 text-amber-800'],
              ['Approved', approvedCount, 'bg-emerald-50 border-emerald-200 text-emerald-800'],
              ['On Leave Today', onLeaveTodayCount, 'bg-cyan-50 border-cyan-200 text-cyan-800'],
              ['Rejected', rejectedCount, 'bg-rose-50 border-rose-200 text-rose-800'],
            ].map(([label, value, cls]) => (
              <div key={String(label)} className={`rounded-2xl border p-4 shadow-sm ${cls}`}>
                <span className="text-[10px] font-bold uppercase tracking-wider block">{label}</span>
                <span className="text-2xl font-extrabold mt-1 block">{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700 mb-3">Your 2026 Annual Leave Balance Quotas</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {balances.map(bal => (
              <div key={bal.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm text-center">
