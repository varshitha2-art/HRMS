...
export const AuditLogs: React.FC = () => {
  ...
  useEffect(() => {
    fetchLogs();
  }, [search, filterModule, filterAction, fromDate, toDate]);
  ...
  const getSeverity = (log: AuditLog) => {
    if (['DELETE', 'REJECT'].includes(log.action)) return { label: 'CRITICAL', cls: 'bg-rose-50 text-rose-700 border-rose-200' };
    if (['UPDATE', 'STATUS_UPDATE'].includes(log.action) || log.module === 'SETTINGS') return { label: 'WARNING', cls: 'bg-amber-50 text-amber-800 border-amber-200' };
    if (['APPROVE', 'GENERATE', 'DISBURSE'].includes(log.action)) return { label: 'SUCCESS', cls: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
    return { label: 'INFO', cls: 'bg-slate-50 text-slate-700 border-slate-200' };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-300 rounded-xl text-xs text-emerald-800 font-bold">
          <Shield className="w-4 h-4 text-emerald-600" /> Protected Audit Trail
        </div>
      </div>

      {/* Filter Bar */}
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
        ...
        <div>
          ...
          <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-800 focus:outline-none focus:border-amber-500 focus:bg-white" />
        </div>

        <div>
          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">To date</label>
          <input type="date" value={toDate} onChange={e => setToDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-800 focus:outline-none focus:border-amber-500 focus:bg-white" />
        </div>
              ...
              <tbody className="divide-y divide-slate-200 text-slate-700">
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      {(() => { const sev = getSeverity(log); return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${sev.cls}`}>{sev.label}</span>; })()}
                    </td>
        ...
        {selectedLog && (
          <div className="space-y-4 text-xs">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
              ...
              <div className="flex justify-between items-center"><span className="text-slate-500">Severity:</span>{(() => { const sev = getSeverity(selectedLog); return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${sev.cls}`}>{sev.label}</span>; })()}</div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-slate-700 font-bold">Payload JSON</label>
                <button type="button" onClick={() => navigator.clipboard?.writeText(selectedLog.details || '')} className="inline-flex items-center gap-1 text-[10px] font-bold text-slate-600 hover:text-amber-700"><Copy className="w-3.5 h-3.5" /> Copy JSON</button>
              </div>
