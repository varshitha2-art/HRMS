...
export const PayrollPage: React.FC = () => {
  ...
  const [pendingPayrollAction, setPendingPayrollAction] = useState<'APPROVED' | 'PAID' | 'CALCULATED' | null>(null);
  ...
  const payrollExceptions = useMemo(() => {
    const missingAttendance = payrollDetails.filter(i => i.presentDays === 0 && i.paidLeaveDays === 0 && i.workingDays > 0).length;
    const lopEmployees = payrollDetails.filter(i => i.lopDays > 0).length;
    const overtimeEmployees = payrollDetails.filter(i => i.overtimePay > 0).length;
    return { missingAttendance, lopEmployees, overtimeEmployees };
  }, [payrollDetails]);
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedPayroll && (
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-slate-200">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {selectedPayroll.status === 'CALCULATED' && (
                <button
                  onClick={() => setPendingPayrollAction('APPROVED')}
              ...
              {selectedPayroll.status === 'APPROVED' && (
                <>
                  <button
                    onClick={() => setPendingPayrollAction('CALCULATED')}
                  ...
                  <button
                    onClick={() => setPendingPayrollAction('PAID')}
              ...
              {selectedPayroll.status === 'PAID' && (
                <div className="px-3.5 py-1.5 bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-xs font-bold flex items-center gap-1" title="Paid payroll is locked from normal editing">
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Locked after payment
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {['CALCULATED', 'VERIFIED', 'APPROVED', 'PAID'].map((step, idx) => {
              const order: Record<string, number> = { CALCULATED: 1, VERIFIED: 2, APPROVED: 3, PAID: 4 };
              const current = order[selectedPayroll.status] || 0;
              const active = current >= order[step];
              return <div key={step} className={`rounded-xl border px-3 py-2 text-[10px] font-bold uppercase tracking-wider ${active ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-400'}`}><span className="block">{idx + 1}</span>{step}</div>;
            })}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between mb-2">
                <div><h3 className="text-xs font-extrabold text-slate-900">Payroll validation</h3><p className="text-[11px] text-slate-500 mt-0.5">Review exceptions before approval.</p></div>
                <AlertCircle className="w-4 h-4 text-amber-600" />
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-white border border-slate-200 rounded-xl p-2"><span className="block text-lg font-extrabold text-rose-700">{payrollExceptions.missingAttendance}</span><span className="text-[9px] uppercase font-bold text-slate-500">Missing attendance</span></div>
                <div className="bg-white border border-slate-200 rounded-xl p-2"><span className="block text-lg font-extrabold text-amber-700">{payrollExceptions.lopEmployees}</span><span className="text-[9px] uppercase font-bold text-slate-500">LOP employees</span></div>
                <div className="bg-white border border-slate-200 rounded-xl p-2"><span className="block text-lg font-extrabold text-cyan-700">{payrollExceptions.overtimeEmployees}</span><span className="text-[9px] uppercase font-bold text-slate-500">With overtime</span></div>
              </div>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <h3 className="text-xs font-extrabold text-amber-900">Approval control</h3>
              <p className="text-[11px] text-amber-800 mt-1">Approved batches should be treated as read-only. Reopening requires an explicit audit action.</p>
            </div>
          </div>
      ...
      />
      <Modal
        isOpen={!!pendingPayrollAction}
        onClose={() => setPendingPayrollAction(null)}
        title={pendingPayrollAction === 'APPROVED' ? 'Approve Payroll Batch' : pendingPayrollAction === 'PAID' ? 'Mark Payroll as Paid' : 'Reopen Payroll Batch'}
        subtitle={selectedPayroll ? `${new Date(selectedPayroll.payrollYear, selectedPayroll.payrollMonth - 1, 1).toLocaleString('default', { month: 'long' })} ${selectedPayroll.payrollYear}` : ''}
        maxWidth="md"
      >
        {pendingPayrollAction && selectedPayroll && (
          <div className="space-y-4 text-xs">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 grid grid-cols-3 gap-3">
              <div><span className="text-[10px] uppercase font-bold text-slate-500">Employees</span><strong className="block text-lg text-slate-900">{selectedPayroll.totalEmployees}</strong></div>
              <div><span className="text-[10px] uppercase font-bold text-slate-500">Deductions</span><strong className="block text-lg text-rose-700">{formatCurrency(selectedPayroll.totalDeductions)}</strong></div>
              <div><span className="text-[10px] uppercase font-bold text-slate-500">Net payout</span><strong className="block text-lg text-amber-800">{formatCurrency(selectedPayroll.totalNet)}</strong></div>
            </div>
            <p className="text-slate-700">{pendingPayrollAction === 'APPROVED' ? 'Approving confirms the payroll figures for this batch and should be followed by controlled disbursement.' : pendingPayrollAction === 'PAID' ? 'Marking the batch as paid records the disbursement state for the entire payroll batch.' : 'Reopening sends the batch back to calculated status for attendance or rate-card adjustments.'}</p>
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-200">
              <button onClick={() => setPendingPayrollAction(null)} className="px-4 py-2 rounded-xl border border-slate-200 bg-slate-100 text-slate-700 font-bold">Cancel</button>
              <button onClick={() => { const action = pendingPayrollAction; setPendingPayrollAction(null); handleApprovePayroll(action); }} className={`px-4 py-2 rounded-xl font-bold text-white ${pendingPayrollAction === 'CALCULATED' ? 'bg-amber-600 hover:bg-amber-700' : pendingPayrollAction === 'PAID' ? 'bg-teal-600 hover:bg-teal-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>Confirm {pendingPayrollAction === 'APPROVED' ? 'Approval' : pendingPayrollAction === 'PAID' ? 'Payment' : 'Reopen'}</button>
            </div>
          </div>
        )}
      </Modal>
