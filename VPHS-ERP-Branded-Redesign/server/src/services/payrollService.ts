...
export const VPHS_STANDARD_RATE_CARD = {
  basic: 6000,
  da: 10000,
  subTotal1: 16000,
  hra: 6783,
...
export interface SalaryBreakdownResult {
  monthly: {
    ...
    // Employer Contributions (PartOf CTC)
    employerPf: number; // P.F @ Employer (13%)
    employerEsi: number; // ESI @ Employer (3.25%)
    bonus: number; // Statutory Bonus (8.33%)
    telanganaLwf: number; // Telangana LWF (0.17)
    gratuity: number;
...
export function calculateSalaryStructure(
  ...
  const isVphsStandard = Math.abs(ctc - VPHS_STANDARD_RATE_CARD.totalMonthlyCtc) < 50 || customRules?.isVphsStandard;

  let basic = customRules?.basic ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.basic : Math.round((ctc * 0.40)));
  let da = customRules?.da ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.da : Math.round(basic * 0.10));
  const subTotal1 = basic + da;
  let hra = customRules?.hra ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.hra : Math.round(basic * 0.40));
  const conveyance = customRules?.conveyance ?? 0;
  const medical = customRules?.medicalAllowance ?? 0;
  const uniform = customRules?.uniformAllowance ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.uniformAllowance : 200);
  ...
  const employerPf = customRules?.employerPf ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.employerPf : Math.min(1950, Math.round((subTotal1 * 13) / 100)));
  // ESI: 3.25% on Basic+DA or Gross = ₹613 standard
  const employerEsi = customRules?.employerEsi ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.employerEsi : Math.round((subTotal1 * 3.25) / 100));
  // Bonus: 8.33% on Basic+DA = ₹1,333 standard
  const bonus = customRules?.bonus ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.bonus : Math.round((subTotal1 * 8.33) / 100));
  const telanganaLwf = customRules?.telanganaLwfEmployer ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.telanganaLwfEmployer : 0.17);
  ...
  const grossSalary = customRules?.grossSalary ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.grossSalary : (baseEarnings + specialAllowance));

  // Auto-calculated Leave Wages (12.5% of Gross = ₹2,848 on ₹22,783)
  const leaveWages = customRules?.leaveWages !== undefined ? customRules.leaveWages : (isVphsStandard ? VPHS_STANDARD_RATE_CARD.leaveWages : Math.round(grossSalary * 0.125));

  const totalEmployerContribution = employerPf + employerEsi + bonus + leaveWages + uniform + telanganaLwf + gratuity + insuranceBenefit;

  // Employee Deductions
  const employeePf = customRules?.employeePf ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.employeePf : Math.round((Math.min(subTotal1, 15000) * 12) / 100));
  const employeeEsi = customRules?.employeeEsi ?? (grossSalary <= 21000 ? Math.round((grossSalary * 0.75) / 100) : (isVphsStandard ? VPHS_STANDARD_RATE_CARD.employeeEsi : 0));
  const professionalTax = customRules?.professionalTax ?? (grossSalary > 15000 ? 200 : grossSalary > 10000 ? 150 : 0);
  const employeeLwf = customRules?.employeeLwf ?? (isVphsStandard ? VPHS_STANDARD_RATE_CARD.employeeLwf : 2);
  ...
  return {
    ...
    annual: {
      ctc: isVphsStandard ? VPHS_STANDARD_RATE_CARD.annualCtc : ctc * 12,
      headcount,
      basic: basic * 12,
      da: da * 12,
...
export async function generateMonthlyPayroll(
  ...
  for (const emp of employees) {
    ...
    let rateCard: any = { ...VPHS_STANDARD_RATE_CARD };
    ...
    } else if (emp.site && (emp.site as any).rateCardJson) {
      try {
        const siteCard = JSON.parse((emp.site as any).rateCardJson);
        rateCard = {
 ...
 */
export async function syncEmployeePayslipFromAttendance(
  employeeId: string,
...
export async function syncAllSiteEmployeesPayslips(siteId: string, month?: number, year?: number) {
