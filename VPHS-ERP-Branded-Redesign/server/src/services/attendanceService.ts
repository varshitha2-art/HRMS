...
export async function calculateAttendanceMetrics(
  ...
  if (shiftId) {
    ...
    const settings = await prisma.attendanceSetting.findFirst();
    if (settings) {
  ...
  if (inTimeStr) {
    ...
    let actualMinutesSinceMidnight: number | null = null;
...
export function parseLocalDate(dateStr: string): { year: number; month: number; day: number; dayOfWeek: number } {
 ...
 */
export async function processWeekendSandwichRule(
  employeeId: string,
  markedDate: string,
  newStatus: string,
  siteId?: string | null,
  ...
  if (dayOfWeek === 5) {
    ...
    if (newStatus === 'ABSENT') {
      ...
      const satAtt = await prisma.attendance.findUnique({
        where: { employeeId_date: { employeeId, date: satDateStr } },
      });
    ...
    if (newStatus === 'ABSENT') {
      ...
      const sunAtt = await prisma.attendance.findUnique({
        where: { employeeId_date: { employeeId, date: sunDateStr } },
      });
 ...
 */
export async function applyWeekendSandwichRuleForMonth(
  month: number,
  year: number,
  siteId?: string
) {
  ...
  if (siteId) empWhere.siteId = siteId;
