...
exports.removeEmployeePhoto = removeEmployeePhoto;
...
async function getEmployees(req, res) {
    try {
        ...
        const { search, departmentId, siteId, status, designationId, page = '1', limit = '50', } = req.query;
        ...
        const [employees, total] = await Promise.all([
            db_1.default.employee.findMany({
                where,
                skip,
                take: limitNum,
                orderBy: [
            ...
            db_1.default.employee.count({ where }),
        ]);
        return (0, response_1.sendSuccess)(res, employees, 'Employees fetched successfully', {
            total,
            page: pageNum,
            limit: limitNum,
...
async function getEmployeeById(req, res) {
    try {
        ...
        const hasAccess = await (0, rbacService_1.canAccessEmployee)(req.user, id);
        if (!hasAccess) {
            return (0, response_1.sendError)(res, 'Access denied: You do not have permission to view this employee record', 403);
        }
        ...
        return (0, response_1.sendSuccess)(res, employee);
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, 500);
    }
...
async function createEmployee(req, res) {
    try {
        if (!req.user || !['SUPER_ADMIN', 'ADMIN', 'HR'].includes(req.user.role)) {
            return (0, response_1.sendError)(res, 'Access denied: Insufficient permissions to create employees', 403);
        }
        const validated = schemas_1.employeeCreateSchema.parse(req.body);
        const { role, salaryCtc, ...empData } = validated;
        // Check duplicate employeeId
        const existing = await db_1.default.employee.findUnique({
            where: { employeeId: empData.employeeId },
        });
        if (existing) {
            return (0, response_1.sendError)(res, `Employee ID ${empData.employeeId} already exists`, 409);
        }
        ...
        if (employee.siteId) {
            await db_1.default.siteAssignment.create({
                data: {
                    employeeId: employee.id,
        ...
        await (0, audit_1.logAuditAction)(req.user?.userId, 'EMPLOYEE', 'CREATE', employee.id, { employeeId: employee.employeeId, name: `${employee.firstName} ${employee.lastName}` }, req.ip);
        return (0, response_1.sendSuccess)(res, employee, 'Employee created successfully', undefined, 201);
    }
    catch (error) {
...
async function updateEmployee(req, res) {
    try {
        if (!req.user || !['SUPER_ADMIN', 'ADMIN', 'HR'].includes(req.user.role)) {
            return (0, response_1.sendError)(res, 'Access denied: Insufficient permissions to edit employees', 403);
        }
        const { id } = req.params;
        ...
        const hasAccess = await (0, rbacService_1.canAccessEmployee)(req.user, id);
        if (!hasAccess) {
            return (0, response_1.sendError)(res, 'Access denied: You do not have permission to update this employee record', 403);
        }
        if (updateData.dob)
        ...
        await (0, audit_1.logAuditAction)(req.user?.userId, 'EMPLOYEE', 'UPDATE', employee.id, { employeeId: employee.employeeId, updatedFields: Object.keys(updateData) }, req.ip);
        return (0, response_1.sendSuccess)(res, employee, 'Employee updated successfully');
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, 400);
    }
...
async function deleteEmployee(req, res) {
    try {
        if (!req.user || !['SUPER_ADMIN', 'ADMIN'].includes(req.user.role)) {
            return (0, response_1.sendError)(res, 'Access denied: Insufficient permissions to delete employees', 403);
        }
        const { id } = req.params;
        const employee = await db_1.default.employee.findUnique({ where: { id } });
        if (!employee) {
            return (0, response_1.sendError)(res, 'Employee not found', 404);
        }
        ...
        await (0, audit_1.logAuditAction)(req.user?.userId, 'EMPLOYEE', 'DELETE', employee.id, { employeeId: employee.employeeId, action: 'Terminated employee' }, req.ip);
        return (0, response_1.sendSuccess)(res, null, 'Employee status marked as TERMINATED');
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, 400);
    }
...
async function updateSalaryStructure(req, res) {
    try {
        if (!req.user || !['SUPER_ADMIN', 'ADMIN', 'HR'].includes(req.user.role)) {
            return (0, response_1.sendError)(res, 'Access denied: Insufficient permissions to modify salary structures', 403);
        }
        const { id } = req.params;
        const data = req.body;
        const employee = await db_1.default.employee.findFirst({
        ...
        const newStructure = await db_1.default.salaryStructure.create({
            data: {
                employeeId: employee.id,
                ctc,
                basic,
        ...
        await (0, audit_1.logAuditAction)(req.user?.userId, 'PAYROLL', 'UPDATE', employee.id, { employeeId: employee.employeeId, newCtc: ctc, netSalary }, req.ip);
        // ⚡ Real-Time Auto-Sync: Automatically update employee payslip as per attendance without manual approval
        try {
            const now = new Date();
            await (0, payrollService_1.syncEmployeePayslipFromAttendance)(employee.id, now.getMonth() + 1, now.getFullYear());
            // Also sync August 2026 if batch exists
            const augBatch = await db_1.default.payroll.findUnique({
                where: { payrollMonth_payrollYear: { payrollMonth: 8, payrollYear: 2026 } }
            });
            if (augBatch) {
                await (0, payrollService_1.syncEmployeePayslipFromAttendance)(employee.id, 8, 2026);
            }
        }
        catch (syncErr) {
            console.warn('Payslip auto-sync on salary structure update failed:', syncErr);
        }
        return (0, response_1.sendSuccess)(res, newStructure, 'Custom salary structure updated successfully');
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, 400);
    }
...
async function uploadEmployeePhoto(req, res) {
    try {
        ...
        if (!isSelf && !isElevated) {
            if (req.file)
                fs_1.default.unlinkSync(req.file.path);
            return (0, response_1.sendError)(res, 'Access denied: You can only update your own photo', 403);
        }
        let photoUrl = '';
        if (req.file) {
            ...
            return (0, response_1.sendError)(res, 'No photo file or photoUrl provided', 400);
        }
        const updated = await db_1.default.employee.update({
            where: { id: employee.id },
            data: { photoUrl },
            include: {
                department: true,
        ...
        await (0, audit_1.logAuditAction)(req.user.userId, 'EMPLOYEE', 'UPDATE', employee.id, { employeeId: employee.employeeId, action: 'UPDATE_PHOTO', photoUrl }, req.ip);
        return (0, response_1.sendSuccess)(res, updated, 'Employee photo updated successfully');
    }
    catch (error) {
...
async function removeEmployeePhoto(req, res) {
    try {
        if (!req.user)
            return (0, response_1.sendError)(res, 'Unauthorized', 401);
        const { id } = req.params;
        ...
        await (0, audit_1.logAuditAction)(req.user.userId, 'EMPLOYEE', 'UPDATE', employee.id, { employeeId: employee.employeeId, action: 'REMOVE_PHOTO' }, req.ip);
        return (0, response_1.sendSuccess)(res, updated, 'Employee photo removed successfully');
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, 500);
    }
