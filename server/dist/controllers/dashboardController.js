...
async function getDashboardSummary(req, res) {
    try {
        ...
        const accessibleSiteIds = await (0, rbacService_1.getAccessibleSiteIds)(user);
        const accessibleEmpIds = await (0, rbacService_1.getAccessibleEmployeeIds)(user);
        // =========================================================================
        // 1. EMPLOYEE DASHBOARD
        // =========================================================================
        if (user.role === 'EMPLOYEE') {
            const employee = user.employee || (user.employeeId
                ? await db_1.default.employee.findUnique({
            ...
            const [monthlyAttendances, todayAttendance, leaveBalances, latestPayrollItem, myDocuments,] = await Promise.all([
                db_1.default.attendance.findMany({
                    where: {
                        employeeId: employee.id,
                        date: { startsWith: `${currentYear}-${String(currentMonth).padStart(2, '0')}` },
                    },
                }),
                db_1.default.attendance.findUnique({
                    where: {
                        employeeId_date: {
                            employeeId: employee.id,
                            date: todayStr,
                ...
                db_1.default.leaveBalance.findMany({
                    where: { employeeId: employee.id, year: currentYear },
                    include: { leaveType: true },
                }),
                db_1.default.payrollItem.findFirst({
        ...
        if (user.role === 'SUPERVISOR') {
            const empWhere = await (0, rbacService_1.buildEmployeeWhereClause)(user);
            const attWhere = await (0, rbacService_1.buildAttendanceWhereClause)(user, { date: todayStr });
            const [myTeamCount, todayAttendances, pendingTeamLeaves, teamMembers,] = await Promise.all([
                db_1.default.employee.count({ where: empWhere }),
                db_1.default.attendance.findMany({
                    where: attWhere,
        ...
        if (user.role === 'SITE_MANAGER') {
            ...
            const empWhere = await (0, rbacService_1.buildEmployeeWhereClause)(user);
            const attWhere = await (0, rbacService_1.buildAttendanceWhereClause)(user, { date: todayStr });
            const [mySitesCount, myEmployeesCount, todayAttendances, pendingLeaves, assignedSites,] = await Promise.all([
                db_1.default.site.count({ where: siteWhere }),
                db_1.default.employee.count({ where: { ...empWhere, status: 'ACTIVE' } }),
                db_1.default.attendance.findMany({
                    where: attWhere,
        ...
        // =========================================================================
        const [totalEmployees, activeEmployees, inactiveEmployees, totalSites, todayAttendances, pendingLeaves, expiringDocs, newJoiners, latestPayroll, departmentCounts, siteEmployees, recentAuditLogs,] = await Promise.all([
            db_1.default.employee.count(),
            db_1.default.employee.count({ where: { status: 'ACTIVE' } }),
            db_1.default.employee.count({ where: { status: { in: ['INACTIVE', 'TERMINATED', 'RESIGNED'] } } }),
            db_1.default.site.count({ where: { status: 'ACTIVE' } }),
            db_1.default.attendance.findMany({
                where: { date: todayStr },
                select: { status: true, lateMinutes: true },
            }),
            db_1.default.leaveRequest.count({ where: { status: 'PENDING' } }),
            db_1.default.document.count({
                where: { expiryDate: { lte: thirtyDaysFromNow } },
            }),
            db_1.default.employee.count({
                where: { joiningDate: { gte: thirtyDaysAgo } },
            }),
            db_1.default.payroll.findFirst({
        ...
        const pastWeekAttendances = await db_1.default.attendance.findMany({
