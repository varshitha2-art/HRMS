...
export const SITE_COORDINATE_PRESETS: Record<string, { lat: number; lng: number; radius: number }> = {
...
export function verifySiteGeofence(
  ...
  if (
    ...
    return {
      ...
      siteLng: 0,
      rejectionReason:
        'Punch rejected: Real-time device GPS coordinates are mandatory. Please enable location services and grant location permission on your device.',
    };
  }

  // 2. Resolve Site / Facility coordinates
  let siteLat: number = site?.latitude ?? 0;
  let siteLng: number = site?.longitude ?? 0;
  let allowedRadius: number = site?.geofenceRadius ?? 100;

  // Check presets if site has 0 or null coordinates
  if ((!siteLat || !siteLng) && site?.siteCode && SITE_COORDINATE_PRESETS[site.siteCode]) {
    const preset = SITE_COORDINATE_PRESETS[site.siteCode];
    siteLat = preset.lat;
    siteLng = preset.lng;
  ...
  if (!isWithinGeofence) {
    return {
      ...
      siteLat,
      siteLng,
      rejectionReason: `Punch rejected: Location outside geofence perimeter. You are ${Math.round(
        distanceMeters
      )}m away from ${siteName} (Allowed radius: ${allowedRadius}m). Punch is only accepted when physically present at the site.`,
    };
  }
